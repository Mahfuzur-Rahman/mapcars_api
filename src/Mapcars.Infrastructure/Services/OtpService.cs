    ...
    ILogger<OtpService> logger) : IOtpService
{
    ...
    private const int EmailExpiryMinutes = 10;

    public async Task<string> CreateAndSendPhoneOtpAsync(string userType, string phone, CancellationToken ct = default)
    {
        var code = await CreateCodeAsync(userType, "phone", phone, PhoneExpiryMinutes, ct);
    ...
    {
        var code = await CreateCodeAsync(userType, "email", emailAddr, EmailExpiryMinutes, ct);
    ...
    private async Task<string> CreateCodeAsync(
        string userType, string provider, string identifier, int expiryMinutes, CancellationToken ct)
    {
